...
const STATUS_META: Record<
  string,
  { color: string; icon: ComponentProps<typeof MaterialCommunityIcons>["name"] }
> = {
...
export default function BookingsScreen() {
  ...
  const [refundsMap, setRefundsMap] = useState<Map<string, PendingRefund>>(
    new Map(),
  );
  ...
  const loadReviews = useCallback(async () => {
    try {
      ...
      const ids = new Set(
        reviews.map((r) => r.bookingReference).filter(Boolean),
      );
      ...
      >
        ...
        <View style={styles.cardBody}>
            ...
            {refund && (
              ...
              >
                ...
                <Text style={styles.refundBadgeText}>
                  {refund.refundStatus === "COMPLETED" ? "Đã hoàn" : "Chờ hoàn"}
                </Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.priceSection}>
            ...
            {typeof item.depositAmount === "number" &&
            ...
            {typeof item.remainingAmount === "number" &&
              item.remainingAmount > 0 && (
                <View style={styles.priceRow}>
                  ...
                  <Text style={[styles.priceSubValue, { color: "#f59e0b" }]}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  if (item.status === "PENDING") {
                    ...
                    showToast(
                      "Chỉ có thể chỉnh sửa khi chưa thanh toán cọc",
                      "warning",
                    );
                  }
                }}
          ...
          {(item.status === "CONFIRMED" || item.status === "CHECKED_IN") &&
            new Date(item.checkOut) > new Date() && (
              <View style={styles.actionRowFull}>
                <TouchableOpacity
                  style={[styles.actionBtn, styles.btnDining, { flex: 1 }]}
                  onPress={() =>
                    navigation.navigate("BookingDining", { bookingId: item.id })
                  }
                >
                  <MaterialCommunityIcons
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    navigation.navigate("Equipment", { booking: item })
                  }
                >
                  <MaterialCommunityIcons name="beach" size={14} color="#fff" />
                  <Text style={styles.btnTextLight}>🏖️ Mượn đồ</Text>
                </TouchableOpacity>
              </View>
            )}
        </View>
...
const styles = StyleSheet.create({
  ...
  actionRowFull: { flexDirection: "row", gap: 8, marginTop: 12, width: "100%" },
  actionBtn: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  ...
  btnEquipment: { backgroundColor: "#7c3aed" },
