...
import {
    refundService,
    type PendingRefund,
} from "@/service/refund/refundService";
...
function parseExperiencesFromSpecialRequests(
      ...
      <View style={styles.timelineContent}>
        <Text
          style={[
            ...
            (done || active) && { color: "#0f172a", fontWeight: "700" },
          ]}
        >
          {label}
        </Text>
        {sublabel ? (
          <Text style={styles.timelineSublabel}>{sublabel}</Text>
        ) : null}
      </View>
    </View>
  );
...
export default function BookingDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (detail) {
        ...
        // Load extra charges
        const charges = await extraChargeService
          .getByBooking(detail.id)
          .catch(() => []);
        setExtraCharges(charges);
        // Load homestay detail
        if (detail.homestayId) {
          publicHomestayService
            .getById(detail.homestayId)
            .then(setHomestay)
            .catch(() => {});
        }
  ...
  const paymentCfg = booking.paymentStatus
    ? (PAYMENT_CFG[booking.paymentStatus] ?? PAYMENT_CFG.UNPAID)
    : null;
  ...
  const images: string[] = homestay?.images?.length
    ? homestay.images
  ...
  const depositPaid = ["DEPOSIT_PAID", "FULLY_PAID"].includes(
  ...
  return (
    <SafeAreaView style={styles.container} edges={[]}>
      ...
      >
        ...
        <View
          style={[styles.statusBanner, { borderLeftColor: statusCfg.color }]}
        >
          <View style={styles.statusBannerLeft}>
            <View
              style={[styles.statusIconWrap, { backgroundColor: statusCfg.bg }]}
            >
              <MaterialCommunityIcons
                name={
            ...
            <View>
              ...
              <Text
                style={[styles.statusBannerValue, { color: statusCfg.color }]}
              >
                {statusCfg.label}
              </Text>
            </View>
          ...
          {paymentCfg && (
            <View style={styles.statusBannerRight}>
              ...
              <Text
                style={[styles.statusBannerValue, { color: paymentCfg.color }]}
              >
                {paymentCfg.label}
              </Text>
            </View>
              ...
              <View style={styles.amenitiesWrap}>
                ...
                {homestay.amenities.length > 6 && (
                  <View style={styles.amenityChip}>
                    <Text style={styles.amenityText}>
                      +{homestay.amenities.length - 6} khác
                    </Text>
                  </View>
                )}
              </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.infoCard}>
            <InfoRow
              ...
              label="Nhận phòng"
              value={new Date(booking.checkIn).toLocaleDateString("vi-VN", {
                weekday: "short",
                day: "2-digit",
                month: "2-digit",
                year: "numeric",
            ...
            <InfoRow
              ...
              label="Trả phòng"
              value={new Date(booking.checkOut).toLocaleDateString("vi-VN", {
                weekday: "short",
                day: "2-digit",
                month: "2-digit",
                year: "numeric",
            ...
            <InfoRow
              ...
              label="Số khách"
              value={`${booking.guestsCount} khách`}
            />
            ...
            {booking.specialRequests && (
              <>
                ...
                {(() => {
                  const experiences = parseExperiencesFromSpecialRequests(
                    booking.specialRequests,
                  );
                        ...
                        {experiences.map((exp, idx) => (
                          ...
                          >
                            <View style={styles.experienceInfo}>
                              ...
                              <Text style={styles.experienceQty}>
                                Số lượng: {exp.qty}
                              </Text>
                            </View>
              ...
              <>
                <View style={styles.priceRow}>
                  <Text style={styles.priceRowLabel}>
                    ₫{homestay.pricePerNight.toLocaleString("vi-VN")} × {nights}{" "}
                    đêm
                  </Text>
                  <Text style={styles.priceRowValue}>
                    ₫{(homestay.pricePerNight * nights).toLocaleString("vi-VN")}
                  </Text>
                </View>
                <Divider style={{ marginVertical: 8 }} />
              </>
                ...
                <>
                  ...
                  <View style={styles.priceRow}>
                    ...
                    <Text
                      style={[
                        styles.priceRowValue,
                        { color: depositPaid ? "#059669" : "#d97706" },
                      ]}
                    >
            ...
            {extraCharges.length > 0 && (
              <>
                ...
                <View style={styles.extraChargesSection}>
                  <Text style={styles.extraChargesSectionTitle}>
                    Các khoản phí khác
                  </Text>
                  {extraCharges.map((charge, idx) => (
                    <View key={charge.id || idx} style={styles.extraChargeRow}>
                      ...
                      <Text style={styles.extraChargeValue}>
                        +₫{charge.amount.toLocaleString("vi-VN")}
                      </Text>
                    </View>
                  ))}
                  <View style={styles.extraChargesTotalRow}>
                    <Text style={styles.extraChargesTotalLabel}>
                      Tổng phí khác
                    </Text>
              ...
              <>
                ...
                <View style={styles.priceRow}>
                  ...
                  <Text
                    style={[
                      styles.priceRowValue,
                      { color: paymentCfg.color, fontWeight: "700" },
                    ]}
                  >
                    {paymentCfg.label}
                  </Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.timelineCard}>
            ...
            <TimelineStep
              ...
              sublabel={
                depositPaid
                  ? `₫${(booking.depositAmount ?? 0).toLocaleString("vi-VN")}`
                  : "Chưa thanh toán"
              }
              done={depositPaid}
              active={isPending}
            ...
            <TimelineStep
              ...
              sublabel={
                fullyPaid
                  ? `₫${(booking.remainingAmount ?? 0).toLocaleString("vi-VN")}`
                  : "Thanh toán trước check-in"
              }
              done={fullyPaid}
              active={isConfirmed && !fullyPaid}
            />
            <View style={styles.timelineLine} />
            <TimelineStep
              step={4}
              label="Check-in & Lưu trú"
              sublabel={
                isCheckedIn
                  ? isCompleted
                    ? "Đã hoàn thành"
                    : "Đang lưu trú"
                  : "Chờ đến ngày check-in"
              }
              done={isCompleted}
              active={isCheckedIn && !isCompleted}
            />
          </View>
        </View>
              ...
              <View
                style={[
                  ...
                  refund.refundStatus === "COMPLETED"
                    ? styles.refundBoxCompleted
                    : styles.refundBoxPending,
                ]}
              >
          ...
          {(booking.status === "CHECKED_IN" ||
            ...
            new Date(booking.checkOut) > new Date() && (
              <TouchableOpacity
                ...
                onPress={() =>
                  navigation.navigate("BookingDining", {
                    bookingId: booking.id,
                  })
                }
              >
                <LinearGradient
                  ...
                  style={styles.diningBtnGradient}
                >
                  <MaterialCommunityIcons
                    ...
                    color="#fff"
                  />
                  <Text style={styles.diningBtnText}>
                    🍽️ Đặt món ăn · Tính vào hóa đơn phòng
                  </Text>
                </LinearGradient>
              </TouchableOpacity>
            )}

          {/* Equipment — chỉ hiện khi đang lưu trú hoặc đã confirm */}
          {(booking.status === "CHECKED_IN" ||
            booking.status === "CONFIRMED") &&
            new Date(booking.checkOut) > new Date() && (
              <TouchableOpacity
                style={styles.equipmentBtn}
                onPress={() => navigation.navigate("Equipment", { booking })}
              >
                <LinearGradient
                  colors={["#7c3aed", "#a855f7"]}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 0 }}
                  style={styles.equipmentBtnGradient}
                >
                  <MaterialCommunityIcons name="beach" size={18} color="#fff" />
                  <Text style={styles.equipmentBtnText}>🏖️ Mượn đồ dùng</Text>
                </LinearGradient>
              </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
  ...
  equipmentBtn: { marginBottom: 10, borderRadius: 14, overflow: "hidden" },
  equipmentBtnGradient: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: 8,
  ...
  equipmentBtnText: { fontSize: 15, fontWeight: "800", color: "#fff" },
});
