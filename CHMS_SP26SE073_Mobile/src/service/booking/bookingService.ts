...
export const bookingService = {
  ...
  async createBooking(payload: {
    ...
    const res = await apiClient.post<any>(apiConfig.endpoints.bookings.create, payload);
    return {
  ...
  async cancelBooking(id: string) {
    const res = await apiClient.post<any>(apiConfig.endpoints.bookings.cancel(id));
    return { success: Boolean(res?.success ?? true), message: res?.message ?? "Da huy booking" };
  },
};
